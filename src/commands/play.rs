...
#[cfg(feature = "dev")]
pub fn run_repl_from_state<R: BufRead, W: Write>(
    reader: &mut R,
    writer: &mut W,
...
pub fn run_repl_with_save_dir<R: BufRead, W: Write>(
    reader: &mut R,
    writer: &mut W,
    seed: u64,
    save_dir: &std::path::Path,
...
pub fn execute(dev_state_path: Option<&str>, save_dir_override: Option<std::path::PathBuf>) {
    ...
    let save_dir = save_dir_override.unwrap_or_else(|| std::path::PathBuf::from("saves"));

    // Dev mode: --dev-state <file> injects an arbitrary pre-crafted GameState.
    #[cfg(feature = "dev")]
    if let Some(path) = dev_state_path {
